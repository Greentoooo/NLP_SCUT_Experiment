...
def dataPreprocessing(dataPath):
    ...
    dataPath = PROJECT_PATH + "\\train_data(preprocess).txt"
    ...
    negetive_list = [
        'nm', 'tm', 'fw', 'laji', 'sb', 'zz', '低能', '全家', '爸', '尼玛', '曰',
        '草', '艹', '傻', '挂', '送', '投', '颂', '狗', '妈','ma', '臭b', '马', '你妹',  '宋', 'gou'
    ]
    positive_list = [
...
def valuation(dataPath, range, classifier):
    ...
    if range < 0 and range >= -1:
        for text in textlist[int(range * len(textlist)):]:
            text = text.strip('\n')  # 去除末尾的\n
            text = text.split('\t')  # 去除空格
            result = (classifier.classify(text[1]) == 'negetive')
            if (result is True):
                if (text[2] == "1"):
                    TP += 1
                elif (text[2] == "0"):
