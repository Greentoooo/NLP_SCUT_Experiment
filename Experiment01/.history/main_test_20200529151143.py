...
def predict(model):
    ...
    ###################################
    TP,FP,TN,FN=0,0,0,0;
    fin = open(g_DATA_FILE, 'r', encoding='utf-8')
    fout = open(g_OUT_PUT_FILE, 'a+', encoding='utf-8')
    fout.write("qid\ttext\tpredict_label\n")
    fin.readline()
    for line in fin.readlines():
        line = line.strip('\n')
        line = line.split('\t')
        predictLabel = int(model.classify(line[1]) == 'negetive')
        fout.write('%s\t%s\t%d\n' % (line[0], line[1], predictLabel))
        if (predictLabel is True):
            if (line[2] == "1"):
                TP += 1
            elif (line[2] == "0"):
                FP += 1
        elif (predictLabel is False):
            if (line[2] == "1"):
                FN += 1
            elif ((line[2] == "0")):
                TN += 1
